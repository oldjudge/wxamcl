...
void GlobalOptions::Init()
{
	...
	wxString s;
		...
		if (!m_workdir.DirExists(s))//first time around?
		{
			...
			src = s;
			src<<wxT("/testscintilla.lua");
			dest = GetWorkDir();
			wxCopyFile(src, dest<<"/scripts/testscintilla.lua");
			src = s;
			src<<wxT("/thread.lua");
			dest = GetWorkDir();
			wxCopyFile(src, dest<<"/scripts/thread.lua");
			src = s;
			src<<wxT("/defaultprofile.lua");
			dest = GetWorkDir();
